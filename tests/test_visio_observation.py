...
class TestReadingARealPackage:
    def test_it_finds_the_pages_in_the_order_visio_numbers_them(self, basedir):
        """`pages.xml` order, which is what Visio numbers by, not filename order.

        The names carry that order. The indices do not: they are generated by
        `enumerate(..., start=1)` and come out `1, 2, 3` however the pages were
        read.
        """
        observation = observation_from_package(f"{basedir}/test1.vsdx")

        assert [(page.index, page.name) for page in observation.pages] == [
            (1, "Page-1"),
            (2, "Page-2"),
            (3, "Page-3"),
        ]
